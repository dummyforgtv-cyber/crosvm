...
impl VfioPlatformDevice {
    ...
    /// Locate the MMIO region that fully contains `[addr, addr+len)`.
    ///
    /// Start-only matching used to reach `region_read`/`region_write`, which
    /// panic when `addr + len` exceeds the region size. Require the whole
    /// access to fit instead.
